...
def reg_server(root,frame1,client_socket,email,name,sex,state,city,passw):
    if(passw=='' or passw==' '):
...
def Register(root,frame1):
    ...
    reg = Button(frame1, text="Register", command = lambda: reg_server(root, frame1, client_socket, email.get(), name.get(), sex.get(), state.get(), city.get(), password.get()), width=10)
